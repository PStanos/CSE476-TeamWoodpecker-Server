...
processXml(stripslashes($_POST['xml']), $user);

function processXml($xmltext, $user){
	$xml = new XMLReader();
	if(!$xml->XML($xmltext)) {
	...
	$query = "UPDATE FlockingGame SET Xml=$xmltextQ, DataRead = 0, DataSender=$userQ WHERE Player1=$userQ OR Player2=$userQ";
